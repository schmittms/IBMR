...
class iterativeIB(object):
    ...
    def step_(self, beta, P_H_cond_V, P_H, longdouble, debug=False, record_loss=False):
        ...
        for i in range(self.iter_steps):
            ...
            x = beta*np.matmul(self.P_E_cond_V.T, np.maximum(-1e10, np.log(P_E_cond_H + 1e-16))).T
            
            #x = np.maximum(x, -700)
            if debug: print(f"Step {i}\n\t x:\t {x}")
            if debug: print('p(e|h)', P_E_cond_H)
            if debug: print('p(v|h)', P_V_cond_H)
    ...
    def run(self, iter_steps, 
        ...
        if pbar_bool: pbar = tqdm(total=len(self.betas))
        ...
        for b, beta in enumerate(self.betas):
            ...
            if compute_hessian:
                ...
                if pbar_bool: pbar.set_description(f'β = {beta:0.2f}, $\\lambda_1$ = {evals[0].real:0.2f}, I = {i_vh:0.2e} (hit={hit_transition})') 
            else:
                if pbar_bool: pbar.set_description(f'β = {beta:0.2f}, I = {i_vh:0.2e} (hit={hit_transition})') 
...
class iterativeIBcomputeAfterKick(iterativeIB):
    def run(self, iter_steps, 
                    betas, 
                    C_H,
                    longdouble=False,
